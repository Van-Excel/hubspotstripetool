...
export default function AuditPage() {
  ...
  const { data, isLoading } = useAuditLogs({ page: String(page) })
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">{formatDate(selected.created)}</span>
              </div>
            ...
            {selected.ip_address && (
              <div className="text-xs text-muted-foreground">IP: {selected.ip_address}</div>
            )}
          </div>
        )}
      </Dialog>
